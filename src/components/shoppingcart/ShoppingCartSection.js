...
function ShoppingCartSection(props) {
  ...
  return (
    <React.Fragment>
      <div className={`cart-bg ${bgClasses}`}></div>
      <CartContext.Consumer>
        {(context) => (
          // If user clicks outside the shopping cart, toggle showCart and close the shopping cart
          <OutsideClickHandler
            isCartShown={context.cart.cart.showCart}
            onOutsideClick={() => {
              context.show.toggleShowCart();
            }}
          >
            <Container
              className={`shopping-cart m-0 px-3 ${classes}`}
              id="shopping-cart-tray"
            >
                ...
                <React.Fragment>
                  ...
                  <Col xs={{ size: 3, offset: 0 }} className="text-right px-4">
                    <Button
                      ...
                      onClick={() => context.show.toggleShowCart()}
                    >
                      &times;
                    </Button>
                  </Col>
